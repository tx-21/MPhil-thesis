...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, index): #idx will iterate between 0 and __len__
        ...
        target = torch.tensor(self.df[["NH3_N", "Colour", "sin_hour", "cos_hour"]][start + self.T : start + self.T + self.S].values)
        ...
        # # exit()
        scaler.fit(_input[:,:2]) #the minmaxscaler can only be applied on arr with dim>=2
        _input[:,:2] = torch.tensor(scaler.transform(_input[:,:2]))
        target[:,:2] = torch.tensor(scaler.transform(target[:,:2]))
