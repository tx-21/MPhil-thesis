...
class CNN(nn.Module):
    ...
    def __init__(self, input_size=1, hidden_dim=1, output_size=2):
...
class RNN(nn.Module):
    """Vanilla RNN"""
    def __init__(self, input_size=2, hidden_size=10, num_layers=1, output_size=2):
...
class GRU(nn.Module):
    ...
    def forward(self, src, device):
        h_t = torch.zeros(self.num_layers,1,self.hidden_size).double()
        out, _ = self.gru((src[:,:,:2]),h_t)
        out = self.fc(out)
        return out


class model_MLP_1(torch.nn.Module):
    def __init__(self, n_input=2, n_hidden=10, n_batch=1, n_output=2):
