...
def inference_archived(path_to_save_predictions, forecast_window, dataloader, device, path_to_save_model, best_model):
    ...
    model = model_LSTM_1().double().to(device)
    model.load_state_dict(torch.load(path_to_save_model+best_model))
    criterion = torch.nn.MSELoss()
    ...
    with torch.no_grad():
        ...
        for plot in range(3):
            ...
            for index_in, index_tar, _input, target in dataloader: #iterate all the samples
                # starting from 1 so that src matches with target (so that the first and the last predicted token match with the target), but has same length as when training
                src = _input.permute(1,0,2).double().to(device)[1:, :, :] # 47, 1, 7: t1 -- t47
                target = target.permute(1,0,2).double().to(device) # t48 - t59
                next_input_model = src
            ...
            src_ammonia = scaler.inverse_transform(src[:,:,0].cpu())
            target_ammonia = scaler.inverse_transform(target[:,:,0].cpu())
            prediction_ammonia = scaler.inverse_transform(all_predictions[:,:,0].detach().cpu().numpy())
            plot_prediction(plot, path_to_save_predictions, src_ammonia, target_ammonia, prediction_ammonia, index_in, index_tar)
