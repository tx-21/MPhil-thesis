...
def log_test_raw(value : float, path_to_save_loss : str, name):

    file_name = "forecast_raw_data.txt"

    path_to_file = path_to_save_loss+ f'{name}_'+file_name
    os.makedirs(os.path.dirname(path_to_file), exist_ok=True)
    with open(path_to_file, "a") as f:
        f.write(str(value)+"\n")
        f.close()
...
def stability_test(metric_all, Exp_num, result_loc, dataset_name):
        ...
        for j in range(Exp_num):
            ...
            epoch_ls[j] = metric_all[j][9]    

        metric_summary_fc1 = np.empty([input_metric_number_fc1*4+1])
        ...
        metric_summary_fc1[1] = round(r2_ls_1.mean(),4)
        metric_summary_fc1[2] = round(test_loss_ls.mean(),4)
        metric_summary_fc1[3] = round(train_loss_ls.mean(),4)
        metric_summary_fc1[4] = round(valid_loss_ls.mean(),4)
        metric_summary_fc1[5]  = epoch_ls[-1]
        metric_summary_fc1[6]  = round(rmse_ls_1.std(),4)
        metric_summary_fc1[7]  = round(rmse_ls_1.max(),4)
        metric_summary_fc1[8]  = round(rmse_ls_1.min(),4)
        metric_summary_fc1[9]  = round(r2_ls_1.std(),4)
        metric_summary_fc1[10] = round(r2_ls_1.max(),4)
        metric_summary_fc1[11] = round(r2_ls_1.min(),4)
        metric_summary_fc1[12] = round(test_loss_ls.std(),4)
        metric_summary_fc1[13] = round(test_loss_ls.max(),4)
        metric_summary_fc1[14] = round(test_loss_ls.min(),4)
        metric_summary_fc1[15] = round(train_loss_ls.std(),4)
        metric_summary_fc1[16] = round(train_loss_ls.max(),4)
        metric_summary_fc1[17] = round(train_loss_ls.min(),4)
        metric_summary_fc1[18] = round(valid_loss_ls.std(),4)
        metric_summary_fc1[19] = round(valid_loss_ls.max(),4)
        metric_summary_fc1[20] = round(valid_loss_ls.min(),4)
