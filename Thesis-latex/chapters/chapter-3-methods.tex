...
Data smoothing was performed using the same method on ammonia and colour datasets. One of the effective ways to remove the noise from the dataset is to apply data smoothing filters. Two filters were applied in this study, Savitzky-Golay (SG) and Exponentially Weighted Moving Average (EWMA) filters.
...
\begin{table}[!ht]
  \centering
  \caption{Final model configuratino.}\label{tab:model-config}
  \begin{NiceTabular}{lccccl}
      ...
      \midrule
      RF   & 24\tabularnote{24 hourly data points were input into the models for trainin.} & - & 3 & 3 & Estimators = 500 \\
...
The settings of the optimzer hyperparameters are listed in the followings:
